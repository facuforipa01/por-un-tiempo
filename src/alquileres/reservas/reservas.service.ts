...
async reservar(desde: Date, hasta: Date, usuarioId: number, deptoId: number) {
   ...
   const reservasConfirmadas = await this.reservaRepository.find(
    {where: {desde: desde, hasta: hasta }, relations: ['departamento']} || {where: {desde: hasta, hasta: desde }, relations: ['departamento']} 
)

console.log(reservasConfirmadas)


 /**   if (reservasConfirmadas.departamento.id == deptoId) {throw new NotFoundException('en esa fecha ya hay una reserva')}
   ...
   if(usuarioFound && !(reservasConfirmadas.departamento.id == deptoId) && deptoFound){
